...
def train(model, num_epochs,dataloader, device, optimizer):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % 50 == 0:
            # 指定保存模型的路径和文件名
            save_path = f'./modelsSL3e-4/model_epoch_{epoch+1}.pth'
            # 保存模型状态字典
...
if __name__ == '__main__':
    ...
    # 确保保存模型的目录存在
    os.makedirs('./modelsSL3e-4', exist_ok=True)
    # 训练模型
    ...
    dataset = datasets.ImageFolder(root=r'2-Data/2-SEM/X5000/AugmentationSL', transform=transform)
